...
from gpiozero import PWMLED  # Modified: Using gpiozero for motor speed control instead of RPi.GPIO
# No need to set mode as in RPi.GPIO, gpiozero handles it automatically
      
speed_pin20 = PWMLED(20, frequency=100)  # set GPIO 20 as output pin using PWMLED on port 20 at 100 Hertz
speed_pin21 = PWMLED(21, frequency=100)  # set GPIO 21 as output pin using PWMLED on port 21 at 100 Hertz
      
val = 1.0  # 1.0 corresponds to 100% duty cycle
speed_pin20.value = val  # start speed_pin20 at 100% duty cycle (on)
speed_pin21.value = val  # start speed_pin21 at 100% duty cycle (on)
    
print("speed set to: ", int(val*100))
#------------------------------------------
...
def main():
    ...
    while True:
        ...
        arr_dur[0]=time.time() - start_total_time
        #----------------------------------------------------
        ...
        arr_dur[1]=time.time() - start_inference
        #----------------------------------------------------
       ...
       #-----------------other------------------------------------
        ...
        arr_dur[2]=time.time() - start_logic
       ...
       # cv2.imshow('Object Tracking - TensorFlow Lite', cv2_im)
        ...
        total_frame_latency = time.time() - start_total_time
        fps = 1.0 / total_frame_latency
        
        # Log these values to a list or file
        latencies = {
            "capture": capture_latency,
            "preprocess": preprocess_latency,
            "inference": inference_latency,
            "logic": logic_latency,
            "total": total_frame_latency,
            "fps": fps
        }
        ...
        arr_dur[2]=time.time() - start_logic
        fps = round(1.0 / (time.time() - start_total_time),1)
        print("*********FPS: ",fps,"************")
